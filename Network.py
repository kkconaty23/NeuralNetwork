...
class Network(object):
    ...
    def Evaluate(self,indata, decision_plotting=0): # evaluates all layers
        ...
        # return 1
        # else:
        # return 0def Train(self, epochs,learningRate, lambda1, gradDescType, batchSize=1):
    def Train(self, epochs,learningRate, lambda1, gradDescType, batchSize=1):
        for j in range(epochs):
            ...
            for i in range(self.X.shape[0]):
                ...
                if (self.lastLayerAF == ActivationType.SOFTMAX):error += -(self.Y[i] * np.log(self.Layers[len(self.numLayers)-1].a+0.001)).sum()
                else:
...
# compute deltas, grads on all layers
                while(lnum >= 0):
                    if (lnum == len(self.numLayers)-1): # last layer
                        if (self.lastLayerAF == ActivationType.SOFTMAX):self.Layers[lnum].delta = -self.Y[i]+ self.Layers[lnum].a
                        else:
